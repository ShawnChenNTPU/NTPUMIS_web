...
class FeedItem {
  ...
  public function __construct( PIS_Helper_Importer $importer, $feed_item ) {
    ...
    $this->_guid   = postcast_importer_secondline_sanitize_feed_value($this->feed_item->guid );
    ...
    $this->episode_number = postcast_importer_secondline_sanitize_feed_value($this->feed_item_itunes->episode );
    $this->season_number  = postcast_importer_secondline_sanitize_feed_value( $this->feed_item_itunes->season );
    $this->filesize       = '' . number_format($this->feed_item->enclosure['length'] / 1048576, 2) . 'M';
    $this->duration       = postcast_importer_secondline_sanitize_feed_value($this->feed_item_itunes->duration );
    ...
    $this->current_post_information[ 'post_title' ]  = postcast_importer_secondline_sanitize_feed_value($feed_item->title);
    ...
    if( $this->importer->import_prepend_title !== '' )
      $this->current_post_information[ 'post_title' ] = str_replace(
        '[podcast_title]',
        postcast_importer_secondline_sanitize_feed_value($this->importer->feedXML->channel->title ),
    ...
    $this->current_post_information[ 'post_excerpt' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item_itunes->subtitle );
  ...
  private function _set_audio_information() {
    ...
    if( $this->importer->import_embed_player && !empty( $this->audio_feed_host ) ) {
      ...
      $this->audio_is_whitelisted = postcast_importer_is_whitelisted_host($this->audio_feed_host, $this->audio_feed_url);
    } else {
    ...
    if( $this->importer->import_content_truncate === false || $this->importer->import_content_truncate >= 1 ) {
      if ( !empty( $this->feed_item->children( 'itunes', true )->summary) && $this->importer->import_content_tag === 'itunes:summary' ) {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item->children('itunes', true )->summary);
      } elseif( !empty( $this->feed_item->children('itunes', true)->encoded ) && $this->importer->import_content_tag === 'content:encoded') {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item->children('content', true )->encoded);
      } elseif( !empty( $this->feed_item->description ) && $this->importer->import_content_tag === 'description' ) {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item->description);
      } elseif( !empty( $this->feed_item->children( 'content', true )->encoded ) ) {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item->children('content', true )->encoded);
      } elseif( !empty( $this->feed_item->description ) ) {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item->description);
      } else {
        $this->current_post_information[ 'post_content' ] = postcast_importer_secondline_sanitize_feed_value($this->feed_item_itunes->summary );
      }
    ...
    if( apply_filters( PODCAST_IMPORTER_SECONDLINE_ALIAS . '_feed_item_has_player_in_content', postcast_importer_secondline_append_player_to_content(), $this ) )
  ...
  private function _set_post_information() {
    ...
    if ( postcast_importer_secondline_has_premium_theme() ) {
    ...
    if ( $this->importer->import_embed_player && $this->audio_is_whitelisted && postcast_importer_secondline_has_premium_theme() ) {
  ...
  private function _handle_image_import() {
    ...
    if( isset( $this->importer->import_images ) && $this->feed_item_itunes && isset( $this->feed_item_itunes->image ) && $this->feed_item_itunes->image->attributes() && isset( $this->feed_item_itunes->image->attributes()->href ) ) {
      $image_url = $this->feed_item_itunes->image->attributes()->href;
    }

    if( $image_url !== false ) {
      $filename = (string) basename(parse_url( $image_url )['path'] );

      $query = "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_value LIKE '%$filename%'";
      $filename_count = intval($wpdb->get_var($query));

      $override_media_hosts = (bool) preg_match('/buzzsprout.com|omny.fm|omnycontent.com/i', $this->audio_feed_url );

      if($filename_count == 0 || $override_media_hosts == true) {
        $img_to_import = (string) $image_url;
        if( !post_exists( $this->current_post_information[ 'post_title' ], '', '', 'attachment' ) || $override_media_hosts == true) {
          $attachment = get_page_by_title( $this->current_post_information[ 'post_title' ], OBJECT, 'attachment');
          if( empty( $attachment ) || $override_media_hosts == true ) {
            add_action( 'add_attachment', [ $this, '_add_attachment_action_image' ] );
            media_sideload_image($img_to_import, $this->current_post_id, $this->current_post_information[ 'post_title' ] );
            remove_action('add_attachment',[ $this, '_add_attachment_action_image' ] );
          }
        }
      } else {
        ...
        $image_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s' LIMIT 1;", $image_src ));

        if( $image_id !== null )
          set_post_thumbnail($this->current_post_id, $image_id);
      }
    }
  }

  // Grab the images and save the image id with the post
  public function _add_attachment_action_image($att_id) {
