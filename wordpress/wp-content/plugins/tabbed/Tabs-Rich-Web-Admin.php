<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
	{
		if(check_admin_referer( 'edit-menu_', 'Rich_Web_Tabs_Nonce' ))
		{
			...
			{
				$wpdb->query($wpdb->prepare("INSERT INTO $table_name3 (id, Tabs_name, Tabs_TID, SubTitles_Count) VALUES (%d, %s, %s, %d)", '', $Rich_Web_Tabs_Name, $Rich_Web_Tabs_Theme, $Rich_Web_Tabs_Count));
			...
			else if(isset($_POST['Rich_Web_Tabs_Update']))
			{
				...
				$wpdb->query($wpdb->prepare("UPDATE $table_name3 set Tabs_name=%s, Tabs_TID=%s, SubTitles_Count=%d WHERE id=%d", $Rich_Web_Tabs_Name, $Rich_Web_Tabs_Theme, $Rich_Web_Tabs_Count, $Rich_Web_Tabs_Upd_ID));
...
<form method="POST" enctype="multipart/form-data">
	...
	<div class="RW_Tabs_Div_Buttons" style="position: relative; width: 99%; height: 50px;">
	...
	<div class='Rich_Web_Tabs_Content'>
		<div class='Rich_Web_Tabs_Content_Data1'>
			...
			<?php for($i=0;$i<count($Rich_Web_Tabs_Dat);$i++){?> 
				<tr class='Rich_Web_Tabs_Content_Table_Tr2 Rich_Web_Tabs_Content_Tr_<?php echo $Rich_Web_Tabs_Dat[$i]->id ?>' >
					<td><?php echo $i+1; ?></td>
					<td><?php echo $Rich_Web_Tabs_Dat[$i]->Tabs_name; ?></td>
					<?php $Rich_Web_Tabs_Acd_TName =$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name5 WHERE id=%d order by id desc limit 1",$Rich_Web_Tabs_Dat[$i]->Tabs_TID));  ?>
					<td><?php echo $Rich_Web_Tabs_Acd_TName[0]->Rich_Web_Tabs_T_T; ?></td>
		...
		<div class="Rich_Web_Tabs_Absolute_Div">
			<div class="Rich_Web_Tabs_Relative_Div">
				<i class="Rich_Web_Tabs_Del_Trash rich_web rich_web-trash"></i>
		...
		<div class='Rich_Web_Tabs_Content_Data2'>
			<table class="Rich_Web_Tabs_ShortTable" style="display: table; float: right;">
				...
				<tr> 
					<td>
						<select  name="Rich_Web_Tabs_Theme" id="Rich_Web_Tabs_Theme" onchange="Rich_Web_Tabs_T_Theme_Changed()">
							<option disabled selected>Select Your Theme . . .</option>
							<?php for($i=0;$i<count($Rich_Web_Tabs_Themes);$i++){ ?>
								<option value="<?php echo $Rich_Web_Tabs_Themes[$i]->id;?>"><?php echo $Rich_Web_Tabs_Themes[$i]->Rich_Web_Tabs_T_T;?></option>
							<?php }?>
						</select>					
			    	</td>
				</tr>
			...
			<div class="Rich_Web_Save_Upd_Tab">
					<input type='button' class='Rich_Web_Tabs_Field_Add' onclick="RichWeb_Tabs_Add_New()"  value='New Tab' >
					<a href="https://wordpress.org/support/plugin/tabbed/" target="_blank"><input type="button"  class="RichWeb_Tabs_Sup_But" value="Support" /></a>
			...
			<div class="Rich_Web_Tabs_PopUp">
				<div class="Rich_Web_Tabs_PopUp_Content">
					<div class="RW_Tabs_Pro_V_W Rich_Web_Tabs_NoDs">
						<span>Pro Version Option</span>
					</div>
				<table class="Rich_Web_Tabs_MainTable" style="">
					<tr>
				...
				<tr class="Rich_Web_Tabs_Bgc_Opt Rich_Web_Tabs_NoDs" style="opacity:0.4;">
					<td colspan="4">
				...
				<tr class="Rich_Web_Tabs_Bgc_Opt Rich_Web_Tabs_NoDs" style="opacity:0.4;" >
					<td colspan="2" >
    					<label class="switch switch-light">
    					    <input class="switch-input" type="checkbox" name="RichWeb_Sub_Bcg_Im" id="RichWeb_Sub_Bcg_Im"  />
    					    <span class="switch-label" data-on="On" data-off="Off"></span> 
    					    <span class="switch-handle"></span>
    					</label>
					</td>
					<td colspan="2">
						<div id="wp-content-media-buttons" class="wp-media-buttons Rich_Web_Tabs_Sub_Add_Img">
							<a href="#" class="button insert-media add_media Rich_Web_Tabs_DisabledBut" style="border:1px solid rgba(0, 73, 107, 0.8); color:rgba(0, 73, 107, 0.8); background-color:#f4f4f4" data-editor="Rich_Web_Sub_ImgSrc_1" title="Add Image" id="Rich_Web_Tabs_Sub_ImgSrc">
								<span class="wp-media-buttons-icon"></span>Add Image
							</a>
						</div>
						<input type="text" style="display:none;" id="Rich_Web_Sub_ImgSrc_1" class='JSlInput JSlInput2'>
						<input type="text" id="Rich_Web_ImgSrc_2" class="JSlInput JSlInput2 Rich_Web_Tabs_ImgSrc_Select" readonly>
					</td>
				</tr>
				<tr>
					<td colspan="4">
			...
			<a class="Rich_Web_Tabs_PopUp_Close" onclick="Rich_Web_Tabs_PopUp_Close()">x</a>
			</div>
			</div>
		</div>
	</div>
